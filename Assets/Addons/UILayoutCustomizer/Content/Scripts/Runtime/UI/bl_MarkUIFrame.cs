...
{
    public class bl_MarkUIFrame : MaskableGraphic
    {
        ...
        {
            if (bl_LayoutCustomizer.Instance == null || !adjustWidth) return;

            var canvas = GetComponentInParent<Canvas>();
            if(canvas != bl_LayoutCustomizer.Instance.ParentCanvas)
            {
                float factor = bl_LayoutCustomizer.Instance.ParentCanvas.scaleFactor / canvas.scaleFactor;
                width *= factor;
            }
        }
